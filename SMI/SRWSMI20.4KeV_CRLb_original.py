...
strIntSourSE_OutFileName = os.path.join(os.getcwd(),strExDataFolderName,'sour_se'+beamline+strBump+'.dat') #file name for output UR intensity Single Electron 
strIntPropSE_OutFileName = os.path.join(os.getcwd(),strExDataFolderName,'prop_se'+beamline+strBump+'.dat') #file name for output intensity propagated Single Electron
strIntPropME_OutFileName = os.path.join(os.getcwd(),strExDataFolderName,'prop_me'+beamline+strBump+'.dat') #file name for output intensity propagated Multi Electron
...
strProfileDataVM         = os.path.join(os.getcwd(),strExDataFolderName,'VM03rms.dat') #VM03rmsL.dat - 127 points, 0.5 m; #VM03rms.dat - 500 points 0.5 m
strProfileDataHKB        = os.path.join(os.getcwd(),strExDataFolderName,'FMsine015.dat') #FMsine015.dat - 127 points 0.5166 m
...
sys.stdout.write('   Setup Magnetic Field for Undulator ... '); sys.stdout.flush()
...
#***********Electron Beam
sys.stdout.write('   Setup Electron Beam for Undulator ... '); sys.stdout.flush()
...
sys.stdout.write('   Setup Stokes mesh ... '); sys.stdout.flush()
...
srwl_uti_save_intens_ascii(opPathDifMOAT, MOAT.mesh, os.path.join(os.getcwd(), strExDataFolderName, 'res_er_mono.dat'), 0, ['', 'Horizontal Position', 'Vertical Position', 'Opt. Path Diff.'], _arUnits=['', 'm', 'm', 'm'])
...
srwl_uti_save_intens_ascii(opPathDifHFMT, HFMT.mesh, os.path.join(os.getcwd(), strExDataFolderName, 'res_er_HFM.dat'), 0, ['', 'Horizontal Position', 'Vertical Position', 'Opt. Path Diff.'], _arUnits=['', 'm', 'm', 'm'])
...
VFMT        = SRWLOptT(200, 288, 6.0e-03, 0.4*sin(3.1415927e-03))#sinusoidal equal to HFM. the origina spec is 0.1, 6.75e-09 both 'h' 'v', angle 6.1086524e-03 rad to correct for vertical.
AuxTransmAddSurfHeightProfile(VFMT, heightProfVFM, 'y', 3.1415927e-03)
opPathDifVFMT = VFMT.get_data(3, 3)
srwl_uti_save_intens_ascii(opPathDifVFMT, VFMT.mesh, os.path.join(os.getcwd(), strExDataFolderName, 'res_er_VFM.dat'), 0, ['', 'Horizontal Position', 'Vertical Position', 'Opt. Path Diff.'], _arUnits=['', 'm', 'm', 'm'])
#================finished VFM slope error=================
#
#================introducing VM slope error==============
heightProfVM = AuxReadInDataColumns(strProfileDataVM, 2, '\t')
VMT        = SRWLOptT(200, 500, 6.0e-03, 0.5*sin(3.1415927e-03))#sinusoidal equal to HFM. the origina spec is 0.1, 6.75e-09 both 'h' 'v', angle 6.1086524e-03 rad to correct for vertical.
AuxTransmAddSurfHeightProfile(VMT, heightProfVM, 'y', 3.1415927e-03)
opPathDifVMT = VMT.get_data(3, 3)
srwl_uti_save_intens_ascii(opPathDifVMT, VMT.mesh, os.path.join(os.getcwd(), strExDataFolderName, 'res_er_VM.dat'), 0, ['', 'Horizontal Position', 'Vertical Position', 'Opt. Path Diff.'], _arUnits=['', 'm', 'm', 'm'])
#================finished VFM slope error=================
#VFMT = SRWLOptT()
...
if beamline=='ES1' and bump and BMmode == 'Norm':
  ...
  # BML     = SRWLOptC( [   D_APE_MOA,   MOAT,   D_MOA_HFM,   HFML,   HFMT,   D_HFM_VFM,   VFML,   VFMT,   D_VFM_SSA,   SSA,   D_SSA_ES1 ],
   #                    [ ppD_APE_MOA, ppMOAT, ppD_MOA_HFM, ppHFML, ppHFMT, ppD_HFM_VFM, ppVFML, ppVFMT, ppD_VFM_SSA, ppSSA, ppD_SSA_ES1 ])
    BML     = SRWLOptC( [   D_APE_MOA,   MOAT,   D_MOA_HFM,   HFML,   HFMT,   D_HFM_VFM,   VFML,   VFMT,   D_VFM_VM,   VMT,   D_VM_SSA,   SSA,   D_SSA_ES1 ],
                        [ ppD_APE_MOA, ppMOAT, ppD_MOA_HFM, ppHFML, ppHFMT, ppD_HFM_VFM, ppVFML, ppVFMT, ppD_VFM_VM, ppVMT, ppD_VM_SSA, ppSSA, ppD_SSA_ES1 ])
...
sys.stdout.write('   Performing Single Electron calculation ... '); sys.stdout.flush()
srwl.CalcElecFieldSR(wfr, 0, magFldCnt, arPrecParSpec)
sys.stdout.write('done\n')

sys.stdout.write('   Saving Single Electron UR Intensity ... '); sys.stdout.flush()
...
sys.stdout.write('   Performing Single Electron Radiation Propagation ... '); sys.stdout.flush()
srwl.PropagElecField(wfr, BML)
sys.stdout.write('done\n')

sys.stdout.write('   Saving Single Electron Propagated Intensity ... '); sys.stdout.flush()
arI = array('f', [0]*wfr.mesh.nx*wfr.mesh.ny) #"flat" array to take 2D intensity data
srwl.CalcIntFromElecField(arI, wfr, 6, 0, 3, wfr.mesh.eStart, 0, 0)
...
radStokesProp = srwl_wfr_emit_prop_multi_e(elecBeam, magFldCnt, meshInitPartCoh, 1, 0.01, nMacroElec, nMacroElecAvgOneProc, nMacroElecSavePer,
                                           strIntPropME_OutFileName, sampFactNxNyForProp, BML) #BML, 1) for angluar representation 
sys.stdout.write('done\n')
